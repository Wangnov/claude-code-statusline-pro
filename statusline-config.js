...
示例:
  ...
  showConfig() {
    ...
    console.log('');
    console.log('🎨 颜色配置:');
    console.log(`  Token颜色: 安全=${config.components.tokens.colors.safe} | 警告=${config.components.tokens.colors.warning} | 危险=${config.components.tokens.colors.danger}`);
    console.log(`  状态颜色: 就绪=${config.components.status.colors.ready} | 思考=${config.components.status.colors.thinking} | 工具=${config.components.status.colors.tool}`);
  ...
  previewTheme(themeName) {
    ...
    try {
      ...
      const testData = {
        model: { id: "claude-sonnet-4", display_name: "Claude Sonnet 4" },
        workspace: { project_dir: "/Users/test/awesome-project", current_dir: "/Users/test/awesome-project" },
  ...
  validateConfig() {
    ...
    try {
      ...
      if (warningThreshold >= dangerThreshold) {
        console.error(`❌ 阈值配置错误: warning(${warningThreshold}) 应小于 danger(${dangerThreshold})`);
        valid = false;
      }
  ...
  run() {
    ...
    switch (command) {
      case 'init':
        this.initConfig(options.includes('--config') ? 
          options[options.indexOf('--config') + 1] : null);
        break;
