...
import { realTimePreviewSelector, createThemeSelector, createLanguageSelector } from './realtime-preview-selector.js';
...
async function testThemeSelector(): Promise<void> {
  ...
  const mockThemePreview: PreviewCallback = async (choice, index) => {
...
async function testLanguageSelector(): Promise<void> {
  ...
  const mockLanguagePreview: PreviewCallback = async (choice, index) => {
...
async function testErrorHandling(): Promise<void> {
  ...
  const errorPreview: PreviewCallback = async (choice, index) => {
...
async function testPerformance(): Promise<void> {
  ...
  const performancePreview: PreviewCallback = async (choice, index) => {
    previewCount++;
...
async function testDisabledOptions(): Promise<void> {
  ...
  const choices: Choice[] = [
    ...
    { name: '禁用选项2', value: 'disabled2', description: '这个选项也被禁用', disabled: '功能未实现' },
    { name: '可用选项2', value: 'available2', description: '这个选项也可以选择' },
  ];

  const disabledPreview: PreviewCallback = async (choice, index) => {
