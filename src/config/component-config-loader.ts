...
export class ComponentConfigLoader {
  ...
  static async loadComponentConfig(
    ...
    try {
      const configDir = this.resolveComponentsDir(baseDir);
      const configPath = join(configDir, `${componentName.toLowerCase()}.toml`);
      
      // 检查缓存 | Check cache
      const cached = this.getFromCache(configPath);
      if (cached) {
        return {
          success: true,
          config: cached,
      ...
      // 缓存结果 | Cache result
      await this.setToCache(configPath, config);
  ...
  static async loadAllComponentConfigs(
    ...
    try {
      ...
      if (enabledComponents && enabledComponents.length > 0) {
        ...
        const files = await this.scanComponentFiles(configDir);
      ...
      const loadPromises = componentNames.map(async (componentName) => {
        const result = await this.loadComponentConfig(componentName, baseDir);
  ...
  private static async parseConfigFile(filePath: string): Promise<ComponentMultilineConfig> {
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new Error(`配置格式错误: ${error.errors.map(e => e.message).join(', ')}`);
      }
  ...
  private static getFromCache(filePath: string): ComponentMultilineConfig | null {
    ...
    const now = Date.now();
    if (now - item.loadTime > this.CACHE_TTL) {
   ...
   * 设置到缓存 | Set to cache
   */
  private static async setToCache(filePath: string, config: ComponentMultilineConfig): Promise<void> {
    try {
