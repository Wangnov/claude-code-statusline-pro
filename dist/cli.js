...
var init_schema = __esm({
  "src/config/schema.ts"() {
    ...
    AutoDetectSchema = z.union([z.boolean(), z.literal("auto")]);
    BaseComponentSchema = z.object({
    ...
    TokenThresholdsSchema = z.object({
    ...
    TokenComponentSchema = BaseComponentSchema.extend({
      ...
      context_window: z.number().default(2e5),
      progress_bar_width: z.number().default(10),
      progress_bar_chars: z.object({
    ...
    StatusIconsSchema = z.object({
      ready: z.string().default("\u2705"),
      thinking: z.string().default("\u{1F4AD}"),
      tool: z.string().default("\u{1F527}"),
      error: z.string().default("\u274C"),
      warning: z.string().default("\u26A0\uFE0F")
    });
    StatusColorsSchema = z.object({
    ...
    AdvancedSchema = z.object({
      cache_enabled: z.boolean().default(true),
      recent_error_count: z.number().min(1).default(5),
      git_timeout: z.number().min(100).default(1e3),
    ...
    TemplateConfigSchema = z.object({
      description: z.string().optional(),
      style: StyleSchema.partial().optional(),
      components: ComponentsSchema.partial().optional()
    }).passthrough();
    TemplatesSchema = z.record(z.string(), TemplateConfigSchema).optional();
...
__export(detector_exports, {
  ...
  detect: () => detect,
  getCapabilityInfo: () => getCapabilityInfo
});
...
var ConfigLoader = class {
  ...
  async save(config, configPath) {
    const targetPath = configPath || this.configPath || path.join(process.cwd(), "statusline.config.toml");
...
var BranchComponent = class extends BaseComponent {
  branchConfig;
  constructor(name, config) {
    super(name, config);
...
var BranchComponentFactory = class {
...
var ModelComponent = class extends BaseComponent {
  modelConfig;
  constructor(name, config) {
    super(name, config);
...
var ModelComponentFactory = class {
  createComponent(name, config) {
...
var ProjectComponent = class extends BaseComponent {
  projectConfig;
  constructor(name, config) {
    super(name, config);
...
var ProjectComponentFactory = class {
  createComponent(name, config) {
...
var StatusComponent = class extends BaseComponent {
  statusConfig;
  cachedStatus = null;
...
var StatusComponentFactory = class {
  createComponent(name, config) {
...
var TokensComponent = class extends BaseComponent {
  tokensConfig;
  ...
  parseTranscriptFile(transcriptPath, context) {
    let fileExists = false;
    try {
      fileExists = existsSync2(transcriptPath) && statSync2(transcriptPath).isFile();
    } catch (_error) {
      return null;
    }
    if (!fileExists) {
...
var TokensComponentFactory = class {
  createComponent(name, config) {
...
program.name("claude-code-statusline-pro").description(
  "Enhanced statusline for Claude Code with live preview and interactive configuration"
).version("2.0.0-beta.1").argument("[preset]", "preset string like PMBT (Project, Model, Branch, Tokens)").option("-p, --preset <preset>", "component preset override").option("-t, --theme <theme>", "theme name (minimal, verbose, developer)").option("--no-colors", "disable colors output").option("--no-emoji", "disable emoji output").option("--no-icons", "disable Nerd Font icons").option("-c, --config <path>", "custom config file path").option("-d, --debug", "debug mode with verbose output").option("-m, --mock <scenario>", "use mock data scenario (dev, critical, error)").action(async (preset, options) => {
  try {
    const configLoader2 = new ConfigLoader();
    let config = await configLoader2.load(options.config);
    ...
    if (options.colors === false || options.emoji === false || options.icons === false) {
      config.style = {
        ...
        enable_nerd_font: options.icons === false ? false : config.style?.enable_nerd_font || "auto",
    ...
    if (options.mock) {
      ...
      const result2 = await generator.generate(mockData);
      console.log(result2);
      return;
    }
...
program.command("config").description("interactive configuration with live preview").option("-f, --file <path>", "config file path").option("-r, --reset", "reset to default configuration").action(async (options) => {
  try {
    if (options.reset) {
...
program.command("theme").description("theme management and selection").argument("[name]", "theme name to apply (minimal, verbose, developer)").action(async (name) => {
  try {
    if (name) {
...
program.command("validate").description("validate configuration file").argument("[file]", "config file path").action(async (file) => {
  try {
    const configLoader2 = new ConfigLoader();
    const _config = await configLoader2.load(file);
...
program.command("doctor").description("diagnose environment and configuration").action(async () => {
  try {
    const { TerminalDetector: TerminalDetector2 } = await Promise.resolve().then(() => (init_detector(), detector_exports));
    const detector = new TerminalDetector2();
...
async function readStdinData() {
  const { parseJson: parseJson2 } = await Promise.resolve().then(() => (init_parser(), parser_exports));
  return new Promise((resolve, reject) => {
    let data = "";
    if (!process.stdin.isTTY) {
      process.stdin.setEncoding("utf8");
      ...
      process.stdin.on("end", () => {
        try {
          if (data.trim()) {
            const parseResult = parseJson2(data.trim());
...
async function resetConfiguration(configPath) {
  const confirm_reset = await confirm2({
  ...
  const configLoader2 = new ConfigLoader();
  await configLoader2.resetToDefaults(configPath);
  console.log(formatCliMessage("success", "\u914D\u7F6E\u5DF2\u91CD\u7F6E\u4E3A\u9ED8\u8BA4\u503C"));
}
async function applyTheme(themeName) {
  const configLoader2 = new ConfigLoader();
  await configLoader2.applyTheme(themeName);
  console.log(formatCliMessage("success", `\u5DF2\u5E94\u7528\u4E3B\u9898: ${themeName}`));
}
async function startThemeSelector() {
  const theme = await select2({
    message: "\u9009\u62E9\u4E3B\u9898\uFF1A",
    choices: [
      { name: "\u7B80\u6D01\u4E3B\u9898 - \u6E05\u723D\u7B80\u5355", value: "minimal" },
      { name: "\u8BE6\u7EC6\u4E3B\u9898 - \u8BE6\u7EC6\u4FE1\u606F", value: "verbose" },
